...
try
    ...
    try
        ...
        mag_image_prep = mag_m0(round(mag_window_cen(2)/mag_bin)-mag_window_size_bin:...
            round(mag_window_cen(2)/mag_bin) + mag_window_size_bin, ...
    ...
    % Marginal centroid for sanity checking
    mag_marginal_x = sum(mag_ready);
    mag_marginal_y = sum(mag_ready,2)';
    mag_marginal_sum = sum(mag_marginal_x);
    if (mag_marginal_sum~= 0)
        mag_marginal_v = (1:length(mag_ready))' - (length(mag_ready)+1)/2;
        mag_marginal_xcen = (mag_marginal_x * mag_marginal_v)/mag_marginal_sum;
        mag_marginal_ycen = (mag_marginal_y * mag_marginal_v)/mag_marginal_sum;
    else
    ...
    mag_marginal_xcen = mag_marginal_xcen*mag_bin; mag_mx = mag_mx*mag_bin;
    mag_marginal_ycen = mag_marginal_ycen*mag_bin; mag_my = mag_my*mag_bin;
    ...
    if ishandle(fh2)==0 % Need to create the figure for the guider image.
        ...
        set(fh2,'name','Guide Box','numbertitle','off','pointer',...
            'cross','busy','queue','inter','off','closerequestfcn','1;');
        f21 = imagesc(-mag_window_size:mag_window_size, -mag_window_size:...
            mag_window_size, zeros([2*mag_window_size+1,2*mag_window_size+1]));
    ...
    set(f21,'XData',-mag_window_size:mag_window_size,'YData', -mag_window_size:mag_window_size);
    ...
    if get(f31,'value') == 1 % show pixel markers
        ...
        set(f30,'xdata',(mag_cscatter-mag_window_size_bin-1)*mag_bin,'ydata',(mag_rscatter-mag_window_size_bin-1)*mag_bin);
    else
